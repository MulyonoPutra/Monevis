...
export class UserFormComponent implements OnInit, OnDestroy {
  ...
  findAllGroup(): void {
    this.groupService.findAll().pipe(takeUntil(this.destroyed)).subscribe({
      next: (response) => {
  ...
  findOne(): void {
    this.userService.findById(this.routeId).pipe(takeUntil(this.destroyed)).subscribe({
      next: (response: any) => {
  ...
  onUpdate(): void {
    this.userService.update(this.routeId, this.formCtrlValue).pipe(takeUntil(this.destroyed)).subscribe({
      next: () => { },
      error: (error: HttpErrorResponse) => {
        console.error(error);
      },
  ...
  onCreate(): void {
    this.userService.create(this.formCtrlValue).pipe(takeUntil(this.destroyed)).subscribe({
      next: () => { },
      error: (error: HttpErrorResponse) => {
        console.error(error);
      },
